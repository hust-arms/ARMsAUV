...
void usbl_cb(const armsauv_msgs::UsblNotRaw::ConstPtr& msg);
...
/* usbl info */
armsauv_msgs::UsblNotRaw usbl_input;
mutex usbl_input_mutex;
armsauv_msgs::UsblNotRaw getUsblMsg()
{
...
void usbl_cb(const armsauv_msgs::UsblNotRaw::ConstPtr& msg){
    lock_guard<std::mutex> guard(usbl_input_mutex);
    usbl_input.position.x = msg->position.x;
    usbl_input.position.y = msg->position.y;
    usbl_input.position.z = msg->position.z;
    usbl_input.pose.x = msg->pose.x;
    usbl_input.pose.y = msg->pose.y;
    usbl_input.pose.z = msg->pose.z;
}
...
void control_loop()
{
	...
	double target_x = usbl_input.position.x;
    double target_y = usbl_input.position.y;
    double refer_y = 0;
...
void timer_cb(const ros::TimerEvent &event)
{
    ...
    target_x = usbl_input.position.x;
    target_y = usbl_input.position.y;
    target_z = usbl_input.position.z;
