...
namespace armsauv_docking{
	...
	// private_nh.param("period", period_, 0.05);
        ...
        /* Create components */
        usbl_pub_ = nh.advertise<armsauv_msgs::UsblNotRaw>(output_topic_, 1);
	...
	*/
	...
	std::cout << "[usbl_sim]:" << "roll:" << roll
        ...
        transc_msg_.position.x = d_x;
        transc_msg_.position.y = d_y;
        transc_msg_.position.z = d_z;
        transc_msg_.pose.x = roll;
        transc_msg_.pose.y = pitch;
        transc_msg_.pose.z = yaw;
    }
