function [ROBNew] = disturbance_dynamic(ROB,deltar,deltab,deltas,tau_dx, tau_dy, tau_dz, tau_dphi, tau_dtheta, tau_dpsi)
%% 全局变量声明
%%艇体本体参数
...
d_z = (a_tq*tau_dz-a_zq*tau_dtheta)/(a_zw*a_tq-a_zq*a_tw); % z轴扰动项
d_theta = (a_zw*tau_dtheta-a_tw*tau_dz)/(a_zw*a_tq-a_zq*a_tw); % 纵倾扰动项
...
d_y = -(a_pr*tau_dy-a_yr*tau_dpsi)/(a_yv*a_pr-a_pv*a_yr); % y轴扰动项
d_psi = -(a_yv*tau_dpsi-a_pv*tau_dy)/(a_yv*a_pr-a_pv*a_yr);% 偏航扰动项
...
ROBdot_v = b_y + b_ydr*deltar + d_y; % 简化水平面运动状态加入扰动项
% ROBdot_w = b_z + b_zb*deltab + b_zs*deltas; 
ROBdot_w = b_z + b_zb*deltab + b_zs*deltas + d_z; % 简化垂直面运动状态加入扰动项
ROBdot_p = 0; 
% ROBdot_q = b_t + b_tb*deltab + b_ts*deltas; 
ROBdot_q = b_t + b_tb*deltab + b_ts*deltas + d_theta;% 简化垂直面运动状态加入扰动项
% ROBdot_r = b_p + b_pdr*deltar;  
ROBdot_r = b_p + b_pdr*deltar + d_psi; % 简化水平面运动状态加入扰动项
