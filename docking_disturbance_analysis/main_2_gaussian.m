...
while t < tfinal
    ...
    [tau_dx, tau_dy, tau_dz, tau_dphi, tau_dtheta, tau_dpsi, is_qt_disturb] = disturb_cal_gaussian(QTx(k), QTy(k), QTz(k), x_auvb, y_auvb, z_auvb,  ROBx(k), ROBy(k), ROBz(k), num, ROBphi(k), ROBtheta(k), ROBpsi(k));
