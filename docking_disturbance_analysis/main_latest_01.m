...
%% 全局变量定义
%%艇体本体参数
global m L W B x_B y_B z_B x_G y_G z_G I_xx I_yy I_zz rad L_qt W_qt H_qt;
...
dt=0.1; tfinal=300; t=0; k=1;  %%步长/总仿真时长/仿真时间/仿真周期数
%% 控制目标量
...
QTx(1)=100; QTy(1)=10; QTz(1)=-10.0;
%x_qt=150; y_qt=1.5; z_qt=0;
u_const_qt=2.5*0.514;
...
%% 理想AUV位置
IROBx = zeros(1,N); IROBy = zeros(1,N); IROBz = zeros(1,N);
...
while t < tfinal
    ...
    [tau_dx, tau_dy, tau_dz, tau_dphi, tau_dtheta, tau_dpsi, is_qt_disturb] = disturb_cal_latest(QTx(k), QTy(k), QTz(k), x_auvb, y_auvb, z_auvb, ROBx(k), ROBy(k), ROBz(k), num, ROBphi(k), ROBtheta(k), ROBpsi(k), L_qt, W_qt, H_qt, t);
    ...
    k = k+1;
    QTx(k)=QTx(k-1)+u_const_qt*dt; QTy(k)=QTy(k-1); QTz(k)=QTz(k-1);%更新潜艇状态
    IROBx(k)=IROBx(k-1)+u_const*dt; IROBy(k)=IROBy(k-1); IROBz(k)=IROBz(k-1);
...
hold on
plot3(IROBx,IROBy,IROBz,'--','Color','g','LineWidth',1.5);
...
%%  叠加扰动曲线
figure('Name','扰动曲线','NumberTitle','off')
subplot(3,1,1)
numv = 1:1:num;
